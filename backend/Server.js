  ...
  _configureRoutes() {
    ...
    this.app.post('/api/products', async (req, res) => {
      try {
        ...
        const savedProduct = await product.save(db);
        res.json({ success: true, data: savedProduct.toSafeObject() });
      } catch (error) {
    ...
    this.app.post('/api/recipes', async (req, res) => {
      ...
      if (!product_id || !Array.isArray(ingredients) || ingredients.length === 0) {
        return res.status(400).json({ success: false, error: 'Producto e ingredientes son requeridos' });
      }

      try {
        const product = await new Product({ id: product_id }).findById(db);
        if (!product) {
        ...
        const validIngredients = ingredients
          ...
          .filter((ingredient) => ingredient.ingredient_id && ingredient.quantity_required > 0);

        if (validIngredients.length === 0) {
          return res.status(400).json({ success: false, error: 'Debe agregar al menos un ingrediente válido' });
        }

        let recipe = await new Recipe().findByProductId(db, product_id);

        await db.transaction(async (client) => {
          if (!recipe) {
            recipe = await new Recipe({ product_id }).save(client);
          } else {
            recipe = new Recipe({ id: recipe.id, product_id });
          }

          const ingredientIds = validIngredients.map((ingredient) => ingredient.ingredient_id);
          if (ingredientIds.length > 0) {
            const deletePlaceholders = ingredientIds.map((_, index) => `$${index + 2}`).join(', ');
            await client.query(
              `DELETE FROM recipe_ingredients WHERE recipe_id = $1 AND ingredient_id NOT IN (${deletePlaceholders})`,
          ...
          for (const ingredient of validIngredients) {
            await recipe.addIngredient(client, ingredient.ingredient_id, ingredient.quantity_required, client);
          }
        });

        const savedRecipe = await new Recipe({ id: recipe.id }).findById(db);
        res.json({ success: true, data: savedRecipe.toSafeObject() });
      } catch (error) {
